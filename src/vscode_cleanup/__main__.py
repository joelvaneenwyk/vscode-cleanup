...
def get_extension_path(search_path):
    if os.path.isdir(os.path.join(search_path, 'extensions')):
        extensions_path = os.path.join(search_path, 'extensions')        
    else:
        extensions_path = search_path
    return os.path.abspath(extensions_path)
    ...
    for extension in os.listdir(extensions_path):  # Iterate through each file/directory in the extensions directory
        ...
        if os.path.isdir(extension_path):  # Check if the current item is a directory
            try:
                ...
                # Store the version in the dictionary under the extension name
...
def remove_duplicates(duplicate_extensions, latest_versions, extensions_path):
    ...
    if action_for_duplicates:
        deprecated_path = os.path.join(extensions_path, ".duplicates")       
        ...
        for name, versions in duplicate_extensions.items():
            for version in versions:
                if version != latest_versions[name]:
                    ...
                    new_extension_path = os.path.join(deprecated_path, f"{name}-{version}")
...
def main():
    ...
    extensions_path = get_extension_path(args.extensions_path)
